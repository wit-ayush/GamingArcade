...
$("#"+ 1).css("background-color",	$("#"+ randomCard).css("background-color"));
  ...
  setTimeout(function() { console.log("sucesfully meshed"); $(".map2").show();
  hideCards(); }, 2000);}
//GAME LOGIC
var gameScore = 0;
var gameTimer = 1;
...
function gameLogic(_id){
  ...
  if(gameTimer === 2 && previousCard != _id){
    gameTimer++;
    // guess correct
  if ($("#"+ _newId).css("background-color") === guessColor){console.log("PARTY WIN");gameScore++; $(".score").css("animation","float 0.4s ease-in-out").text(gameScore);$("#"+ _id).css("background-color", $("#"+ _newId).css("background-color"));gameTimer = 1; $("#"+ _id).attr("onClick",""); $("#"+ previousCard).attr("onClick","");
                                 return;} 
  //guess wrong
  else if($("#"+ _newId).css("background-color") != guessColor){console.log("LOSE");gameScore--; $(".score").css("animation","float 0.1s ease-in-out").text(gameScore); 
  $("#"+ _id).css("background-color", $("#"+ _newId).css("background-color"));
  setTimeout(function() { $("#"+ _id).css("background-color","rgb(247, 230, 180)" );
  $("#"+ previousCard).css("background-color","rgb(247, 230, 180)" );gameTimer = 1; }, 1000);return;
  };}
  return;
}
